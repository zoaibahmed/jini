...
│   └── Dockerfile              # Production multi-stage Next.js Docker build
│
...
│   │   ├── auth/               # JWT signin, signup, silent refresh, & password resets
...
│   └── Dockerfile              # Multi-stage NestJS production container
│
...
- **Responsible Component**: [dashboard/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/page.tsx)
- **Features**: 
  - Welcomes the driver with their active name (`Alex Mercer`).
...
- **Responsible Component**: [dashboard/compliance/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/compliance/page.tsx)
- **Features**:
  ...
  - **Compliance Resolvers**: Contains interactive button hooks to trigger manual completions, updating the database.

#### 1.3 Active Shift Optimizer
- **Responsible Component**: [dashboard/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/page.tsx) (overview panel)
- **Features**:
...
- **Responsible Component**: [dashboard/optimizer/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/optimizer/page.tsx)
- **Features**:
  ...
  - **Optimizer Schedule**: Recommends peak wave driving blocks (e.g., JFK International Shift, LGA Domestic Wave).

#### 1.5 Secure Document Vault
- **Responsible Component**: [dashboard/documents/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/documents/page.tsx)
- **Features**:
  ...
  - **View Layouts**: Switches seamlessly between grid layouts and list layouts.
  - **Metadata Manager**: Allows drivers to update file names, note descriptions, tags, and set exact expiration dates.
  - **Interactive Previews**: A pop-up dialog renders document metadata, S3 keys, and includes a secure download link.
  - **Category Categorizer**: Filters files by category: TLC License, Driver License, Insurance, Vehicle Registration, Inspection Reports, Drug Test, Tax Documents, Tickets.

#### 1.6 SaaS Billing & Subscriptions
- **Responsible Component**: [dashboard/billing/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/billing/page.tsx)
- **Features**:
  ...
  - **Cancellation & Resume**: Drivers can toggle off auto-renewal ("Cancel Subscription") or turn renewal back on ("Resume Renewal") with instantaneous database synchronization.
  - **Promo Coupons**: Validates promotional code strings (`JINI50` applies a 50% discount; `WELCOME10` applies a flat $10 deduction).
  - **Invoice Ledger**: Lists past billing receipts. Includes download button triggers.

#### 1.7 Support Disputes Center
- **Responsible Component**: [dashboard/support/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/support/page.tsx)
- **Features**:
  ...
  - **Read Receipts**: Emits a `markRead` event on ticket activation to update message statuses.

#### 1.8 AI Co-pilot Floating Assistant Widget
- **Responsible Component**: [dashboard/layout.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/layout.tsx)
- **Features**:
  ...
  - **Quick Suggestions**: Buttons to auto-trigger common questions ("JFK surge status", "inspections due", "earnings log").
  - **RBAC Locking**: If a driver's subscription does not include `AI_COPILOT` access, the widget displays a lock warning, advising them to visit the Billing Portal to upgrade.
...
- **Responsible Component**: [dashboard/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/page.tsx) (admin tabs)
- **Features**:
...
- **Responsible Component**: [dashboard/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/page.tsx) (payments tab)
- **Features**:
...
- **Responsible Component**: [dashboard/support/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/support/page.tsx) (agent view)
- **Features**:
  ...
  - **Live Chat Pane**: Full real-time WebSocket chat room to interact with the driver.

#### 2.5 Voice Call Center Dashboard
- **Responsible Component**: [dashboard/voice/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/voice/page.tsx)
- **Features**:
  ...
  - **Manual Agent Transfer**: Features a transfer button to immediately route a call from the AI agent to a live staff member's telephone client.
  ...
  - **Transcripts Pane**: Displays full text conversation logs of past calls, separating speaking turns between driver, AI, and agent. Includes a form to attach admin notes to the call record.

#### 2.6 AI Prompt Monitor Console
- **Responsible Component**: [dashboard/page.tsx](file:///c:/Users/NC/Desktop/jini/frontend/src/app/dashboard/page.tsx) (AI tab)
- **Features**:
  ...
  - **Execution Ledger**: Logs all AI queries, detailing inputs, confidence scores, latencies, and billing costs.

---

## ⚙️ Backend Module Architecture

The NestJS backend API is designed around structured, highly cohesive modules that manage data validation, authentication, and integration flow pipelines.

```mermaid
graph TD
    A[NestJS Gateway main.ts] --> B[AuthModule]
    A --> C[DriverModule]
    A --> D[BillingModule]
    A --> E[SupportModule]
    A --> F[VoiceModule]
    ...
    F --> Twilio[Twilio API]
    G --> OpenAI[OpenAI API]
    G --> ElevenLabs[ElevenLabs API]
    H --> S3[AWS S3 Vault]
```

### 1. `auth` (Authentication & Security)
- **Responsibilities**: Registers new users, processes secure logins using bcrypt, handles silent token refresh rotation, and manages password resets.
- **Security Failsafes**: Leverages short-lived JSON Web Tokens (JWT) in HTTP-only cookies and implements role Guards (`DRIVER`, `SUPPORT`, `ADMIN`, `SUPERADMIN`) to restrict administrative API access.

### 2. `driver` (Driver Profile & Compliance)
- **Responsibilities**: Fetches driver profile metrics, vehicles directory link structures, and coordinates active compliance checks.
- **Compliance Rules**: Automatically updates checklist statuses to `COMPLETED` when matching documents are parsed and validated.

### 3. `billing` (SaaS Payments Layer)
- **Responsibilities**: Syncs billing plans, manages mock Stripe invoice generation, validates discount promo coupons, and processes subscription status transitions.
- **Webhook Provisioner**: Processes incoming billing updates to automatically lock or unlock subscription-restricted features.

### 4. `support` (Dispute Ticket Gates)
- **Responsibilities**: Coordinates ticket dispute submissions and assigns support agents.
- **WebSocket Gateway**: Integrates real-time WebSocket connection handling via Socket.io, routing user-typing notifications, chat messages, and read receipts.

### 5. `voice` (Telephony Automation)
- **Responsibilities**: Houses the Twilio webhook targets for voice call flows.
- **AI State Machine Routing**: Directs callers dynamically (WELCOME → INTENT MENU → INFO DISPATCH → AGENT TRANSFER). Integrates OpenAI for speech-to-text response generation and ElevenLabs for text-to-speech output.
- **Scheduled Callbacks**: Generates an urgent support ticket automatically if a caller requests a callback during non-business hours.

### 6. `copilot` (AI Assistant & RAG FAQs)
- **Responsibilities**: Manages the prompt directives editor, knowledge base FAQs, and AI performance auditing.
- **RAG Search Engine**: Scans FAQs, scores documents based on query keyword matches, and injects relevant articles into the prompt context.
- **Escalation Trigger**: Creates a support ticket automatically if the AI response confidence score drops below `0.4`.

### 7. `document` (Vault & OCR Compliance)
- **Responsibilities**: Allocates S3 pre-signed upload URLs and handles document deletion.
- **OCR Parser Simulator**: Simulates scanning TLC Licenses or Drug Screening documents. It creates `OCRResult` records and automatically marks corresponding compliance items as `COMPLETED` in the database.

---

## 🔄 Asynchronous Background Workers (BullMQ)

Heavy operations are processed asynchronously outside the request-response cycle using Redis-backed BullMQ job queues:

1. **`emails`**: Handles email delivery (e.g., verification tokens, password reset links).
2. **`uploads`**: Processes uploaded documents, triggering OCR parsing and database updates.
3. **`ai_jobs`**: Performs background transcription, translation, and sentiment analysis on voice recordings.
4. **`reminders`**: Scans the database daily to check compliance expiration dates and send notifications.
5. **`notifications`**: Dispatches real-time web socket alerts or SMS updates.
6. **`voice_tasks`**: Performs post-call processing, such as saving audio logs and archiving transcripts.

---

## 🛠️ Local Development Setup

Follow these steps to run the JNI Solutions platform in your local development environment.
...
1. Navigate to the backend directory and copy the environment configuration:
   ```bash
   cd backend
   ```
2. Configure the variables inside your local `backend/.env` file:
   ```env
   DATABASE_URL="postgresql://postgres:ZS3045987@localhost:5432/jini_db?schema=public"
   REDIS_HOST="localhost"
   REDIS_PORT="6379"
   PORT=5000
   JWT_SECRET="production_level_jwt_secret_key"
   BACKEND_URL="http://localhost:5000"
   ```
3. Install dependencies, generate database schemas, seed initial values, and start the development server:
   ```bash
   npm install
   npx prisma generate
   npx prisma db push
   ...
   npm run start:dev
   ```
*Interactive Swagger API documentation is now accessible at `http://localhost:5000/api/docs`.*
...
## 🔑 Initial Demo Login Credentials

You can use the following pre-seeded credentials to log in:

- **Password (for all accounts)**: `ZS3045987`
- **Driver Dashboard**: `driver@jnisolutions.com`
- **Support Agent Console**: `support@jnisolutions.com`
- **Admin Panel**: `admin@jnisolutions.com`
- **Superadmin Console**: `superadmin@jnisolutions.com`

---

## 🛡️ Audit & Security Failsafes
...
- **Granular RBAC Engine**: All API endpoints and WebSocket channels are guarded with role-based validation, preventing unauthorized route tampering.
- **Disaster Recovery Backup**: Database backups are archived daily to a secure, private S3 bucket. Backups older than 30 days are automatically rotated and deleted.
